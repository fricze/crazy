    ...
    for (const item of cssNumericValue.values) {
      ...
      for (const item1 of values) {
        ...
        for (const item2 of newValues) {
          ...
          temp.push([
            item1[0] * item2[0],
            productOfTwoUnitMaps(item1[1], item2[1]),
          ]);
        }
      }
...
export function convertCSSUnitValue(cssUnitValue, unit) {
  ...
  return new CSSUnitValue(
    (oldValue * compatibleUnitGroup.ratios[oldUnit]) /
      compatibleUnitGroup.ratios[unit],
    unit,
  );
}
...
export function multiplyTypes(type1, type2) {
  ...
  const finalType = {
    ...type1,
    percentHint: type1.percentHint ?? type2.percentHint,
  };
...
function toNAryAstNode(operatorToken, first, second) {
  ...
  const type = ["+", "-"].includes(operatorToken.value)
    ? "ADDITION"
    : "MULTIPLICATION";
...
function reifyMathExpression(num) {
  ...
  if (num.name === "min" || num.name === "max") {
    ...
    const values = num.values
      .filter(
        (value) =>
          !(value instanceof WhitespaceToken || value instanceof CommaToken),
      )
...
function reifyNumericValue(num) {
  ...
  if (
    num instanceof CSSFunction &&
    ["calc", "min", "max", "clamp"].includes(num.name)
  ) {
