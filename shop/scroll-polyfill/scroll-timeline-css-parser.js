...
export class StyleParser {
      ...
      try {
        if (target.matches(current.selector)) {
          if (
            ...
            current["animation-name"] == animationName
          ) {
            return {
  ...
  getScrollTimelineOptions(timelineName, target) {
    const anonymousTimelineOptions = this.getAnonymousScrollTimelineOptions(
      timelineName,
      target,
    );
    ...
    for (let i = this.sourceSelectorToScrollTimeline.length - 1; i >= 0; i--) {
      ...
      if (options.name == timelineName) {
        const source = this.findPreviousSiblingOrAncestorMatchingSelector(
          target,
          options.selector,
        );
  ...
  getViewTimelineOptions(timelineName, target) {
    const anonymousTimelineOptions = this.getAnonymousViewTimelineOptions(
      timelineName,
      target,
    );
    if (anonymousTimelineOptions) return anonymousTimelineOptions;
    ...
    for (let i = this.subjectSelectorToViewTimeline.length - 1; i >= 0; i--) {
      ...
      if (options.name == timelineName) {
        const subject = this.findPreviousSiblingOrAncestorMatchingSelector(
          target,
          options.selector,
        );
        if (subject) {
  ...
  handleScrollTimelineProps(rule, p) {
    ...
    const hasAnimationTimeline = rule.block.contents.includes(
      "animation-timeline:",
    );
    ...
    if (hasAnimationName)
      animationNames = this.extractMatches(
        rule.block.contents,
        RegexMatcher.ANIMATION_NAME,
      );
    ...
    if (hasAnimation) {
      this.extractMatches(rule.block.contents, RegexMatcher.ANIMATION).forEach(
        (shorthand) => {
  ...
  saveSourceSelectorToScrollTimeline(rule) {
    ...
    const hasScrollTimelineName = rule.block.contents.includes(
      "scroll-timeline-name:",
    );
    const hasScrollTimelineAxis = rule.block.contents.includes(
    ...
    if (hasScrollTimeline) {
      const scrollTimelines = this.extractMatches(
        rule.block.contents,
        RegexMatcher.SCROLL_TIMELINE,
      );
      for (const st of scrollTimelines) {
        ...
        if (parts.length == 1) {
          ...
          else (options.axis = parts[1]), (options.name = parts[0]);
        }
    ...
    if (hasScrollTimelineName) {
      const names = this.extractMatches(
        rule.block.contents,
        RegexMatcher.SCROLL_TIMELINE_NAME,
      );
    ...
    for (let i = 0; i < timelines.length; i++) {
      if (axes.length) timelines[i].axis = axes[i % timelines.length];
    }
  ...
  saveSubjectSelectorToViewTimeline(rule) {
    ...
    const hasViewTimelineName = rule.block.contents.includes(
      "view-timeline-name:",
    );
    const hasViewTimelineAxis = rule.block.contents.includes(
      "view-timeline-axis:",
    );
    const hasViewTimelineInset = rule.block.contents.includes(
      "view-timeline-inset:",
    );
    ...
    if (hasViewTimeline) {
      const viewTimelines = this.extractMatches(
        rule.block.contents,
        RegexMatcher.VIEW_TIMELINE,
      );
    ...
    if (hasViewTimelineName) {
      const names = this.extractMatches(
        rule.block.contents,
        RegexMatcher.VIEW_TIMELINE_NAME,
      );
      for (let i = 0; i < names.length; i++) {
        if (i < timelines.length) {
          // longhand overrides shorthand
    ...
    if (hasViewTimelineInset)
      insets = this.extractMatches(
        rule.block.contents,
        RegexMatcher.VIEW_TIMELINE_INSET,
      );
    ...
    for (let i = 0; i < timelines.length; i++) {
      if (insets.length) timelines[i].inset = insets[i % timelines.length];
  ...
  saveRelationInList(rule, timelineNames, animationNames) {
    ...
    if (hasAnimationRange)
      animationRanges = this.extractMatches(
        rule.block.contents,
        RegexMatcher.ANIMATION_TIME_RANGE,
      );
  ...
  saveAnonymousTimelineName(part) {
    ...
    if (part.startsWith("scroll(")) {
      this.anonymousScrollTimelineOptions.set(
        name,
        this.parseAnonymousScrollTimeline(part),
      );
    } else {
      this.anonymousViewTimelineOptions.set(
        name,
        this.parseAnonymousViewTimeline(part),
      );
    }
    return name;
  }
  ...
  */
  replaceKeyframesAndGetMapping(rule, p) {
    ...
    for (let i = 0; i < parts.length; i++) {
      const allFrameSelectors = contents.substring(
        parts[i].start,
        parts[i].end,
      );
...
function isAnonymousTimeline(part) {
  return (
    (part.startsWith("scroll") || part.startsWith("view")) && part.includes("(")
  );
}
...
export function removeKeywordsFromAnimationShorthand(anim) {
  return anim
    ...
    .filter(
      (item, index, array) =>
        index == array.length - 1 || !ANIMATION_KEYWORDS.includes(item),
    )
