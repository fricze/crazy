...
import { ProxyAnimation } from "./proxy-animation.js";
import {
  ScrollTimeline,
  ViewTimeline,
  getScrollParent,
...
function createScrollTimeline(anim, animationName, target) {
  ...
  if (options.subject) updateKeyframesIfNecessary(anim, options);

  return {
    timeline: options.source
          ...
          if (tokens.length == 1) {
            ...
            newOffset =
              relativePosition(
                ...
                axis,
                options.inset,
                CSS.percent(parseFloat(tokens[1])),
              ) * 100;
          }
        }
        break;
...
export function initCSSPolyfill() {
  ...
  window.addEventListener("animationstart", (evt) => {
    evt.target
      .getAnimations()
      .filter((anim) => anim.animationName === evt.animationName)
      .forEach((anim) => {
        const result = createScrollTimeline(
          anim,
          anim.animationName,
          evt.target,
        );
        if (result) {
          ...
          if (result.timeline && !(anim instanceof ProxyAnimation)) {
            const proxyAnimation = new ProxyAnimation(
              anim,
              result.timeline,
              result.animOptions,
            );
