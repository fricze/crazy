...
export function simplifyCalculation(root, info = {}) {
  ...
  if (root instanceof CSSUnitValue) {
    ...
    if (root instanceof CSSUnitValue && root.unit === "em" && info.fontSize) {
      root = new CSSUnitValue(
        root.value * info.fontSize.value,
        info.fontSize.unit,
      );
    }
  ...
  switch (root.operator) {
    ...
    case "clamp":
      root = new CSSMathClamp(
        simplifyCalculation(root.lower, info),
        simplifyCalculation(root.value, info),
  ...
  if (root instanceof CSSMathMin || root instanceof CSSMathMax) {
    ...
    if (
      children.every(
        (child) =>
          child instanceof CSSUnitValue &&
          child.unit !== "percent" &&
          isCanonical(child.unit) &&
          child.unit === children[0].unit,
      )
    ) {
      const result = Math[root.operator].apply(
        Math,
  ...
  if (root instanceof CSSMathMin || root instanceof CSSMathMax) {
    ...
    const [numeric, rest] = partition(
      children,
      (child) => child instanceof CSSUnitValue && child.unit !== "percent",
    );
  ...
  if (root instanceof CSSMathSum) {
    ...
    function sumValuesWithSameUnit(values) {
      ...
      const nonNumericValues = values.filter(
        (c) => !(c instanceof CSSUnitValue),
      );

      const summedNumericValues = Array.from(
        groupBy(numericValues, "unit").entries(),
      ).map(([unit, values]) => {
  ...
  if (root instanceof CSSMathProduct) {
    ...
    const [numbers, rest] = partition(
      children,
      (child) => child instanceof CSSUnitValue && child.unit === "number",
    );
    if (numbers.length > 1) {
    ...
    if (children.length === 2) {
      ...
      for (const child of children) {
        if (child instanceof CSSUnitValue && child.unit === "number") {
          ...
          child instanceof CSSMathSum &&
          [...child.values].every((c) => c instanceof CSSUnitValue)
        ) {
          sum = child;
        }
      }
      if (numeric && sum) {
        return new CSSMathSum(
          ...[...sum.values].map(
            (value) =>
              new CSSUnitValue(value.value * numeric.value, value.unit),
          ),
        );
      }
    }
    ...
    if (
      children.every(
        (child) =>
          (child instanceof CSSUnitValue && isCanonical(child.unit)) ||
          (child instanceof CSSMathInvert &&
            child.value instanceof CSSUnitValue &&
